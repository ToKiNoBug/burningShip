...
int main(int argc, char **argv) {
  ...
  norm2_matc1 *norm2_mat = malloc(sizeof(norm2_matc1));
  ...
  compute_frame_norm2c1_range(img, wind, 3000, norm2_mat);

  clk = clock() - clk;

  printf("Finished computation in %lu miliseconds\n", clk);

  write_uncompressed(img, "data.bs_frame");

  const bool ok = write_abstract_matrix(
      &norm2_mat->norm2[0][0], sizeof(bs_float), burning_ship_rows,
      burning_ship_cols, "norm.bs_norm2.gz", true);
